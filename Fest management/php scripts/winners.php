	...
	session_start();
		if(isset($_SESSION['IS_AUTHENTICATED']) && $_SESSION['IS_AUTHENTICATED'] == 1)
		{
			...
			$db = mysql_select_db('test1');
				...
				$q="SELECT * FROM winners";
				$r=mysql_query($q);
				$x=mysql_fetch_array($r);
				
				if(!$x)
				{
					echo '<h3 align="center" class="msg">Events are still running!!!</h3>';
			...
			echo "<div class='container'><div id='event'>
				<h1>Winners</h1><center><table border=1; cellpadding=10; style='border-collapse: collapse;background-color:#ddd;width:90%'>";
			...
			<th style=' border: 1px solid black;padding: 8px; padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: gray;color: white;'>Event</th>
			<th style=' border: 1px solid black;padding: 8px; padding-top: 12px;padding-bottom: 12px;text-align: left;background-color: gray;color: white;'>Rank</th>";
			...
			while ($row = mysql_fetch_array($r1))
			{
				...
				<td style=" border: 1px solid black;padding: 8px;">'.$row['wlogin'].'</td>
				<td style=" border: 1px solid black;padding: 8px;">'.$row['wename'].'</td>
